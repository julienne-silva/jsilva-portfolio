        ...
        <Box sx={{ overflow: 'hidden'}}>
            <Box textAlign="left" paddingY={{ xs: '50px', md: '400px' }} marginLeft={{ xs: '20px', md: '300px' }}>
                <Typography variant={'h5'} fontWeight={'bold'} fontSize={{ xs: '40px', md: '80px' }}>
                    <span style={{ color: BLACK_TEXT_COLOR }}>Julienne </span>
                    <span style={{ color: YELLOW_TEXT_COLOR }}>Silva</span>
                </Typography>
                <Typography variant={'h5'} fontSize={{ xs: '16px', md: '20px' }} marginTop="10px">
                    <span style={{ color: BLACK_TEXT_COLOR }}>I am a passionate <b>junior software developer</b> from the Philippines</span>
                </Typography>
                <Box display={'flex'} justifyContent={'left'} marginTop="10px" marginLeft={{ xs: '-5px', md: '-5px' }}>
                    {['https://www.facebook.com', 'https://www.linkedin.com', 'https://www.instagram.com/', 'https://github.com'].map((url, index) => (
                        <Box
                            key={index}
                            sx={{
                                border: `2px solid ${YELLOW_TEXT_COLOR}`,
                                borderRadius: '50%',
                                padding: '8px',
                                margin: '0 5px',
                                display: 'flex',
                        ...
                        >
                            <a href={url} target="_blank" rel="noopener noreferrer">
                                {index === 0 && <FacebookIcon sx={{ color: YELLOW_TEXT_COLOR }} />}
                                {index === 1 && <LinkedInIcon sx={{ color: YELLOW_TEXT_COLOR }} />}
                                {index === 2 && <InstagramIcon sx={{ color: YELLOW_TEXT_COLOR }} />}
                                {index === 3 && <GitHubIcon sx={{ color: YELLOW_TEXT_COLOR }} />}
                            </a>
                        </Box>
                    ))}
                </Box>
